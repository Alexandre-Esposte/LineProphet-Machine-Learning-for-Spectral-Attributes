from ModelSchema.NeuralNetWork.model import ConvolutionalNet, ConvolutionalAutoEncoder, UNet
...
logging.basicConfig(filename='train_autoencoder.log',level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s', filemode='w')
...
model = torch.load('../models/trained_models/autoencoder.pth')#ConvolutionalAutoEncoder()
sumario =  summary(model, input_size=(batch_size, 1, 100, 100))
...
for epoch in range(epochs):
    perda_treino = train_and_test.train_autoencoder(model, loader_train, optimizer, loss_function)
    perda_valid = train_and_test.test_autoencoder(model, loader_valid, loss_function)
