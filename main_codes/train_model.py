...
for epoch in range(epochs):
    ...
    perda_valid, temperature_loss, pressure_loss  = train_and_test.test(model, loader_valid, loss_function)
    ...
    logging.debug(f"------------------Epoch {epoch + 1}/{epochs} --> train loss: {perda_treino} / test loss: {perda_valid} -------------- "
                  f"temperature loss: {temperature_loss} / pressure loss: {pressure_loss}" 
